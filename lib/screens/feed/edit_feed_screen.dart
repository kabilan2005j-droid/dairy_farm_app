...
class EditFeedScreen extends ConsumerStatefulWidget {
  ...
  const EditFeedScreen({super.key, required this.feed});

  @override
  ConsumerState<EditFeedScreen> createState() =>
      _EditFeedScreenState();
}

class _EditFeedScreenState extends ConsumerState<EditFeedScreen> {
  late TextEditingController _feedNameController;
  ...
  final List<String> _feedTypes = [
    'Hay', 'Silage', 'Grain', 'Concentrate',
    'Straw', 'Mineral Mix', 'Other',
  ];
  ...
  void initState() {
    super.initState();
    _feedNameController =
        TextEditingController(text: widget.feed.feedName);
    _quantityController =
        TextEditingController(text: widget.feed.quantity.toString());
    _unitPriceController =
        TextEditingController(text: widget.feed.unitPrice.toString());
    _totalCostController =
        TextEditingController(text: widget.feed.totalCost.toString());
    _lowStockController = TextEditingController(
        text: widget.feed.lowStockThreshold.toString());
    _supplierController =
        TextEditingController(text: widget.feed.supplierName);
    _notesController =
        TextEditingController(text: widget.feed.notes);
    _feedType = widget.feed.feedType;
    _unit = widget.feed.unit;
    _purchaseDate = widget.feed.purchaseDate;
  }
  ...
  Future<void> _saveChanges() async {
    if (_feedNameController.text.isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Please fill feed name, quantity and price'),
        ),
      );
      return;
    }
